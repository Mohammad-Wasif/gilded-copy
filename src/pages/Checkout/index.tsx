...
import { ArrowLeft, ArrowRight, Lock, ShoppingCart, Truck, CreditCard, BadgeCheck } from 'lucide-react';
...
export default function Checkout() {
  ...
  const handleContinueToPayment = () => {
    ...
    // Basic validation
    if (!formData.shippingName || !formData.shippingAddress || !formData.shippingCity || !formData.shippingState || !formData.shippingZip) {
      setErrorVisible('Please fill out all required shipping fields.');
      return;
    }

    // Store shipping data in sessionStorage so Payment page can access it
  ...
  return (
    <div className="bg-surface text-on-surface font-body selection:bg-primary/10 min-h-screen flex flex-col">
      ...
      <main className="grow max-w-7xl mx-auto px-6 py-12 lg:pl-72 lg:pr-8 w-full">
        ...
        {errorVisible && (
          <div className="mb-8 p-4 bg-error-container text-on-error-container rounded-md font-body text-sm font-semibold border border-error/50">
            {errorVisible}
          </div>
        )}
            ...
            <section className="mb-12">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="col-span-2">
                      ...
                      <input name="shippingName" value={formData.shippingName} onChange={handleChange} className="w-full bg-surface-container-low border-none border-b border-outline-variant focus:ring-0 focus:bg-surface-container-high focus:border-primary transition-all p-3 font-body text-on-surface placeholder:text-outline-variant/50" placeholder="Aditi Sharma" type="text"/>
                    </div>
                    <div className="col-span-2">
                      <label className="block text-[10px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Phone Number</label>
                      <input name="shippingPhone" value={formData.shippingPhone} onChange={handleChange} className="w-full bg-surface-container-low border-none border-b border-outline-variant focus:ring-0 focus:bg-surface-container-high focus:border-primary transition-all p-3 font-body text-on-surface placeholder:text-outline-variant/50" placeholder="+91 98765 43210" type="tel"/>
                    </div>
                    <div className="col-span-2">
                      <label className="block text-[10px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Address line 1 *</label>
                      <input name="shippingAddress" value={formData.shippingAddress} onChange={handleChange} className="w-full bg-surface-container-low border-none border-b border-outline-variant focus:ring-0 focus:bg-surface-container-high focus:border-primary transition-all p-3 font-body text-on-surface placeholder:text-outline-variant/50" placeholder="Suite 402, Heritage Enclave" type="text"/>
                    </div>
                    <div>
                      <label className="block text-[10px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">City *</label>
                      <input name="shippingCity" value={formData.shippingCity} onChange={handleChange} className="w-full bg-surface-container-low border-none border-b border-outline-variant focus:ring-0 focus:bg-surface-container-high focus:border-primary transition-all p-3 font-body text-on-surface placeholder:text-outline-variant/50" placeholder="Jaipur" type="text"/>
                    </div>
                    <div>
                      <label className="block text-[10px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">State *</label>
                      <input name="shippingState" value={formData.shippingState} onChange={handleChange} className="w-full bg-surface-container-low border-none border-b border-outline-variant focus:ring-0 focus:bg-surface-container-high focus:border-primary transition-all p-3 font-body text-on-surface placeholder:text-outline-variant/50" placeholder="Rajasthan" type="text"/>
                    </div>
                    <div>
                      <label className="block text-[10px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">ZIP Code *</label>
                      <input name="shippingZip" value={formData.shippingZip} onChange={handleChange} className="w-full bg-surface-container-low border-none border-b border-outline-variant focus:ring-0 focus:bg-surface-container-high focus:border-primary transition-all p-3 font-body text-on-surface placeholder:text-outline-variant/50" placeholder="302001" type="text"/>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-12 flex justify-start">
                <button
                  ...
                  className="bg-primary text-on-primary px-10 py-4 font-body font-semibold uppercase tracking-widest rounded-md hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-3 group"
                >
                  Continue to Payment
